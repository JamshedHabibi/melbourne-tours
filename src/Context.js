...
class Provider extends Component {
  ...
  componentDidMount() {
    ...
    fetch(`${baseURL}/api/20190906/tour.json?location_ids=Melbourne&count=50&fields=all&order_by=-score`, {
      headers: {
      ...
      .then(res => res.json())
      .then(json => {
        this.setState({ apiIsLoaded: true, apiTours: json, adventures: json.results }, () => {
          this.setTagsToEachTour();
          this.setCostToEachTour();
          this.setState({ selectedTourToView: this.state.adventures[0] }, () => {
            let tourCost = parseFloat(this.state.selectedTourToView.price.amount).toFixed(2);
            this.setState({
          ...
          if (doc.data().id === fire.auth().currentUser.providerData[0].uid) {
            this.setState({ fireStoreUserData: doc.data() }, () => {
              this.setState({
                ...
                photoURL: 'https://png.pngtree.com/svg/20161027/service_default_avatar_182956.png',
  ...
  signUp = e => {
    ...
    fire
      ...
      .then(() => {
        ...
        db.collection('users').doc(`${fire.auth().currentUser.providerData[0].uid}`).set({
  ...
  activateFilterPanel = () => {
    ...
    this.setState({ filterPanelActivated: !currentSetting }, () => {
      if (this.state.filterPanelActivated === false) {
        document.getElementById('adventures-filter-side-panels').style.left = '-8vw';
      } else {
        document.getElementById('adventures-filter-side-panels').style.left = '-150vw';
      }
    });
  };
  ...
  tourPriceFilter = () => {
    ...
    let filteredTours = [];
    for (let i = 0; i < tourListings.results.length; i++) {
      if (tourListings.results[i].cost > minPrice && tourListings.results[i].cost < maxPrice) {
        filteredTours.push(tourListings.results[i]);
      }
    }
    this.setState({ adventures: filteredTours });
  ...
  sortToursByRecommended = () => {
    ...
    originalSort
      ...
      .reverse();
    this.setState({ apiTours: { ...this.state.apiTours, results: originalSort } });
    this.setState({ adventures: originalSort });
  };
  ...
  incrementParticipantCount = () => {
    ...
    let totalTourCost = parseFloat(newCount * this.state.selectedTourToView.price.amount).toFixed(2);
    this.setState({ tourParticipantCount: newCount, totalTourCost: totalTourCost });
  };
  ...
  bookTour = () => {
    ...
    this.setState({
      selectedTourToView: { ...this.state.selectedTourToView, totalBookedCost: this.state.totalTourCost },
    });
    this.setState({
      selectedTourToView: { ...this.state.selectedTourToView, participants: this.state.tourParticipantCount },
    });
    ...
    db.collection('users')
      ...
      .then(snapshot => {
        snapshot.docs.find(doc => {
          if (doc.data().id === fire.auth().currentUser.providerData[0].uid) {
            this.setState({ fireStoreUserData: doc.data() }, () => {
              this.setState({ bookedTours: this.state.fireStoreUserData.bookedTours }, () => {
  ...
  handleVerificationModalClose = () => this.setState({ verificationModalOpen: false });

  handleAccountDetailsModalOpen = () => this.setState({ accountDetailsModalOpen: true });
  handleAccountDetailsModalClose = () => this.setState({ accountDetailsModalOpen: false });
